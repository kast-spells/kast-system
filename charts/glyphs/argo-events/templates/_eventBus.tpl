...
Usage:
- {{- include "argo-events.eventBus" (list $root $glyphDefinition) }}
...
metadata:
  ...
  {{- end }}
spec:
  {{- if eq (default "jetstream" $glyphDefinition.type) "jetstream" }}
  jetstream:
    ...
    affinity:
      {{- . | toYaml | nindent 6 }}
    {{- end }}
  {{- else if eq $glyphDefinition.type "jetstreamExotic" }}
  jetstreamExotic:
    ...
    streamConfig: {{ . }}
    {{- end }}
  {{- else if eq $glyphDefinition.type "nats" }}
  nats:
    native:
      ...
      securityContext:
        {{- . | toYaml | nindent 8 }}
      {{- end }}
  {{- else if eq $glyphDefinition.type "kafka" }}
  kafka:
